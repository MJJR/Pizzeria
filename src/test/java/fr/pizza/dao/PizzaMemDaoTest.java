...
public class PizzaMemDaoTest {
	...
	public void testSaveNewPizza() {
		...
		Assert.assertEquals("Le Premier test (1/3) à échoué", lenght+1, test.getTableau().size());

		Assert.assertEquals("Le Premier test (2/3) à échoué", pizza, test.getTableau().get(lenght));
		
		Assert.assertEquals("Le Premier test (3/3) à échoué",true,test.getTableau().contains(pizza));
	}
	...
	public void testUpdatePizza() {
		...
		Assert.assertEquals("Le Deuxième test a à échoué", pizza, test.getTableau().get(3));
	...
	public void testFindPizzaByCode() {
		...
		Assert.assertEquals("Le Quatrième test aéchoué", pizza, test.findPizzaByCode("ORI"));
